...
K_MSGQ_DEFINE(hrs_queue, sizeof(struct bt_hrs_client_measurement),
              HRS_QUEUE_SIZE, 4);
...
static void
hrs_sensor_location_read_cb(struct bt_hrs_client *hrs_c,
...
static void
hrs_measurement_notify_cb(struct bt_hrs_client *hrs_c,
                          const struct bt_hrs_client_measurement *meas,
                          int err) {
  if (err) {
    printk(
        "Error during receiving Heart Rate Measurement notification, err: %d\n",
        err);
    return;
  }
...
static void discovery_completed_cb(struct bt_gatt_dm *dm, void *context) {
  ...
  if (err) {
    printk("Could not subscribe to Heart Rate Measurement characteristic (err "
           "%d)\n",
           err);
  }
...
static void scan_init(void) {
  ...
  err = bt_scan_filter_add(BT_SCAN_FILTER_TYPE_NAME, "S-Patch EX 001021");
  if (err) {
...
K_THREAD_DEFINE(hrs_notify_thread_id, STACKSIZE, hrs_notify_thread, NULL, NULL,
                NULL, PRIORITY, 0, 0);
