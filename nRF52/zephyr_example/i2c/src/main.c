...
/* STEP 7 - Define the I2C slave device address and the addresses of relevant
 * registers */
...
void main(void) {
  ...
  /* STEP 8 - Setup the sensor by writing the value 0x8C to the Configuration
   * register */
  ...
  while (1) {
    ...
    ret = i2c_write_read(dev_i2c, STTS751_I2C_ADDRESS, &sensor_regs[0], 1,
                         &temp_reading[0], 1);
    if (ret != 0) {
      printk("Failed to write/read I2C device address %x at Reg. %x \n",
             STTS751_I2C_ADDRESS, sensor_regs[0]);
    }
    ret = i2c_write_read(dev_i2c, STTS751_I2C_ADDRESS, &sensor_regs[1], 1,
                         &temp_reading[1], 1);
    if (ret != 0) {
      printk("Failed to write/read I2C device address %x at Reg. %x \n",
    ...
    int temp =
        ((int)temp_reading[1] * 256 + ((int)temp_reading[0] & 0xF0)) / 16;
    if (temp > 2047) {
      temp -= 4096;
    }
